...
class LaGou:
    ...
    def scrape(self, page):
        ...
        logger.info(f"Scraping Page {page} Date")
        try:
            response = requests.post(self.api_url, headers=self.header, cookies=self.cookies, data=data)
            sleep(1)
            if response.status_code == 200:
                return response.json()
            logger.error('get invalid status code %s while scraping %s', response.status_code, self.api_url)
        except Exception as e:
            logger.error(f'Send error on request {e}')

    def parseResponse(self, content):
        dat = content['content']['positionResult']
        # 遍历提取数据
        for data in dat['result']:
            companyFullName = data["companyFullName"]
            # 公司福利
            companyLabelList = ",".join(data["companyLabelList"])
            # 公司规模
            companySize = ",".join(data["companySize"])
            # 城市
            city = data["city"]
            # 地理位置-区
            district = data["district"]
            # 交通路线
            Traffic = data["linestaion"]
            # 地理位置-经纬度
            longitude = str(data["longitude"]) + "," + str(data["matchScore"])
            # 财务阶段
            ...
            # 公司职位优势
            Advantage = data["positionAdvantage"]
            # 职位Lables
            firstType = data["firstType"]
            secondType = data["secondType"]
            thirdType = data["thirdType"]
            jobLab = firstType + "|" + secondType + "|" + thirdType
            # 工作福利
            hiTags = data["hitags"]
            # 薪资范围
            salary = data["salary"]
            # 薪资组成
            salaryMonth = data["salaryMonth"] if data != 0 else 12
            # 技能标签
            skillLables = ','.join(data["skillLables"])
            msg = {
...
def main(page):
    p = LaGou()
    response = p.scrape(page)
    for i in p.parseResponse(response):
        p.save_message(i)
    logger.info(f'Successfully Acquired {page} Page')
